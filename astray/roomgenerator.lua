...
function RoomGenerator:CreateRoom()
    local room = Room:new(math.random(self.minRoomWidth, self.maxRoomWidth),
        math.random(self.minRoomHeight, self.maxRoomHeight))
...
function RoomGenerator:PlaceRooms(dungeon)
    ...
    for roomCounter = 0, self.noOfRoomsToPlace - 1 do
        ...
        for key, currentRoomPlacementLocation in pairs(dungeon:CorridorCellLocations()) do
            local currentRoomPlacementScore = self:CalculateRoomPlacementScore(currentRoomPlacementLocation, room,
                dungeon)
